...
with DAG(dag_id='BigData_YaApi_hits_schedule',
         tags=["ETL на расписании"],
         default_args=default_args,
         schedule_interval='5 1 * * * ',                  # каждый день в 01:05 утра, но тк UTC +3 часа, поэтому в 04:05 утра
    ...
    tlgrm_oneFailed = PythonOperator(
        ...
        op_kwargs={
            'message': f"Упал 🛑 - DAG hits_schedule_{request_id_v}_{start_date_v}_{end_date_v}"}
    )
    ...
    logging_faild = PythonOperator(
        ...
        trigger_rule='one_failed',
        op_kwargs={
            'text': f"DAG hits_schedule_{request_id_v}_{start_date_v}_{end_date_v} - faild" + "\n",
